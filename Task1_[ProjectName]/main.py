...
stats = [
    (col1, "#667eea", str(total),     "Total Tasks"),
    (col2, "#21c354", str(active),    "Active"),
    (col3, "#a0a0a0", str(completed), "Completed"),
    (col4, "#ff4b4b", str(overdue),   "Overdue"),
]
for col, color, val, label in stats:
    with col:
        st.markdown(f"""
    ...
    for task in filtered_tasks:
        ...
        with st.container():
            ...
            <div class="{card_class}" style="border-color:{p_color}">
                <p class="task-title" style="{'text-decoration:line-through;color:#aaa' if is_done else ''}">
                    {"✅ " if is_done else ""}{task['title']}
                </p>
                <div class="task-meta">
                    ...
                    {"<span class='badge' style='background:#fff0f0;color:#ff4b4b'>⚠️ OVERDUE</span>" if is_overdue else ""}
                    📅 Due: {due} &nbsp;|&nbsp; 🕐 Created: {task['created_at'][:10]}
